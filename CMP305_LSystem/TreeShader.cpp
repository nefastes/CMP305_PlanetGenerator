...
TreeShader::TreeShader(ID3D11Device* device, HWND window) : BaseShader(device, window)
{
	initShader(L"tree_vs.cso", L"tree_ps.cso");
}
...
void TreeShader::setShaderParameters(ID3D11DeviceContext* deviceContext, const XMMATRIX& worldMatrix, const XMMATRIX& viewMatrix,
	const XMMATRIX& projectionMatrix, Light* light, const bool is_branch)
{
	HRESULT result;
	...
	settingsPtr->is_branch = is_branch;
	settingsPtr->padding[0] = false;
	settingsPtr->padding[1] = false;
	settingsPtr->padding[2] = false;
	settingsPtr->padding2 = XMFLOAT3(0.f, 0.f, 0.f);
	deviceContext->Unmap(settingsBuffer, 0);
	deviceContext->VSSetConstantBuffers(1, 1, &settingsBuffer);
}

void TreeShader::initShader(const wchar_t* vsFilename, const wchar_t* psFilename)
{
